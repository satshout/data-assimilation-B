...
struct Parameter0
    ...
    function Parameter0(forecast::Function, mode::Mode, num_observed::Int; inflation=1.00, num_ensemble=40, localize=false, L_scale=5)
...
struct SnapShot0
    ...
    function SnapShot0(; x_f=zeros(Float64, 0), x_a=zeros(Float64, 0), Rho_f=zeros(Float64, 0, 0), Rho_a=zeros(Float64, 0, 0), K=zeros(Float64, 0, 0), H=zeros(Int64, 0, 0), M=zeros(Float64, 0, 0), X_f=zeros(Float64, 0, 0), X_a=zeros(Float64, 0, 0), L=zeros(Float64, 0, 0))
...
function integrate(initial_x, Y, Start, Goal, kalman_parameter::Parameter0, lorenz_parameter::Main.Lorenz96.Parameter)
    a = 5.1
    Rho_a = a^2 * Matrix{Int64}(I, lorenz_parameter.num_sites, lorenz_parameter.num_sites)
    snap_shot = SnapShot0(zeros(lorenz_parameter.num_sites), initial_x, zeros(lorenz_parameter.num_sites, lorenz_parameter.num_sites), Rho_a, zeros(lorenz_parameter.num_sites, 1), zeros(1, lorenz_parameter.num_sites), zeros(lorenz_parameter.num_sites, lorenz_parameter.num_sites))
...
function assimilate0(y_observed, observed, t, parameter::Parameter0, lorenz_parameter::Main.Lorenz96.Parameter, snap_shot::SnapShot0)
    """
    y 観測データ
    ...
    next_snapshot = SnapShot0(x_f=x_f, x_a=x_a, Rho_f=Rho_f, Rho_a=Rho_a, K=K, H=H, M=M, X_f=X_f, X_a=X_a, L=snap_shot.L)
...
function SIS(y_observed, t, parameter::Parameter, lorenz_parameter::Main.Lorenz96.Parameter, snap_shot::SnapShot)
    ...
    # 重みの計算
    nx_Xf = [Float64[] for i = 1:m]
    nx_w = zeros(parameter.ensemble_size)

    for k in 1:parameter.ensemble_size
